class Solution {
 public:
  long long maxSubarrays(int n, vector<vector<int>>& conflictingPairs) {
    ...
    int maxLeft = 0;
    int secondMaxLeft = 0;
    // gains[i] := the number of additional valid subarrays we can gain if the restriction at index `i` is removed
    ...
    for (int right = 1; right <= n; ++right) {
      ...
      // If we remove `maxLeft` (the most restrictive conflict), we gain `maxLeft - secondMaxLeft` new subarrays:
